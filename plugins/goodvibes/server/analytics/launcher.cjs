  ...
  try {
    ...
    if (result.repaired) {
      process.stderr.write(
        `[goodvibes:analytics] Repaired runtime dependencies under ${dataRoot}.\n`
      );
    }
  } catch (error) {
    process.stderr.write(
      `[goodvibes:analytics] Automatic runtime dependency repair failed; continuing in degraded mode and retrying on the next startup: ${error instanceof Error ? error.message : String(error)}\n`
    );
  }
...
launch().catch(error => {
  process.stderr.write(
    `[goodvibes:analytics] Launcher failed: ${error instanceof Error ? error.stack || error.message : String(error)}\n`
  );
  process.exitCode = 1;
});
