...
export class Telemetry {
  ...
  public query(filter?: TelemetryQueryFilter): TelemetryRecord[] {
    ...
    if (
      !filter ||
      (!filter.tool && !filter.status && !filter.session_id && !filter.since && !filter.limit)
    ) {
    ...
    if (hasLimit) {
      params.push(Math.max(1, Math.floor(filter.limit as number)));
    }
  ...
  public getSummary(): SessionSummary {
    ...
    const byTool: Record<
      string,
      { calls: number; tokens: number; cache_hits: number; total_ms: number }
    > = {};
    let totalTokens = 0;
  ...
  private resultsToRecords(results: ReturnType<SqlJsDatabase['exec']>): TelemetryRecord[] {
    ...
    return results[0].values.map(row =>
      Telemetry.rowArrayToRecord(row as (string | number | null)[])
    );
  }
