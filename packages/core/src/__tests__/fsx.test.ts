...
describe('resolveInputPath (issue 1 contract)', () => {
  it('passes an absolute input inside the selected base through, with no warning', () => {
    const abs = path.resolve('/abs/project/src/App.tsx');
    const r = resolveInputPath(abs, '/abs/project');
    expect(r.resolved_path).toBe(abs);
    expect(r.warning).toBeUndefined();
  });

  it('rejects an absolute input outside the selected base', () => {
    expect(() => resolveInputPath('/outside/secret.ts', '/abs/project')).toThrow(
...
describe('assertWithinRoot (opt-in boundary, no hidden config)', () => {
  ...
  it('rejects paths that escape the root', () => {
    expect(() => assertWithinRoot('/root/other/a.ts', '/root/project')).toThrow(
