...
describe('honest accounting: meta.token_estimate from rendered payload', () => {
  it('is within 10% for a small fixture', () => {
    expectHonestEstimate(
      payloadText(toCallToolResult(successEnvelope({ file: 'a.ts', lines: 3 })))
    );
  });
  ...
  it('is within 10% on error envelopes too', () => {
    expectHonestEstimate(
      payloadText(toCallToolResult(errorEnvelope('file not found: /tmp/nope.ts')))
    );
  });
