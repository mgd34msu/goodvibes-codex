...
describe('layout_analysis', () => {
  ...
  it('emits the sizing constraint chain only with a selector', async () => {
    ...
    const noSel = parse(
      await handler({ base_path: appDir, file: 'src/Panel.tsx', sections: ['sizing'] })
    );
  ...
  it('omits responsive analysis in this alpha', async () => {
    const env = parse(
      await handler({ base_path: appDir, file: 'src/Panel.tsx', sections: ['responsive'] })
    );
