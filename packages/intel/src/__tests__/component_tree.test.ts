...
describe('component_tree', () => {
  ...
  it('events mode keeps only the two accurate predicates', async () => {
    const env = parse(
      await handler({ base_path: appDir, path: 'src/App.tsx', annotate: ['events'] })
    );
    const app = findNode(env.data!.tree, 'App')!;
  ...
  it('attributes mode is a static overlay of the verified checks', async () => {
    const env = parse(
      await handler({ base_path: appDir, path: 'src/App.tsx', annotate: ['attributes'] })
    );
    const app = findNode(env.data!.tree, 'App')!;
    const attrs = app.attributes!;

    expect(attrs.some(a => a.element.startsWith('img') && a.issues.includes('missing_alt'))).toBe(
      true
    );
    expect(
      attrs.some(a => a.element.startsWith('div') && a.issues.includes('click_without_role'))
    ).toBe(true);
  });

  it('boundaries mode detects a class error boundary with fallback + reset', async () => {
    const env = parse(
      await handler({ base_path: appDir, path: 'src/ErrorBoundary.tsx', annotate: ['boundaries'] })
    );
  ...
  it('honours output.max_tokens by pruning leaves (truncated + effective_caps)', async () => {
    const env = parse(
      await handler({
        base_path: appDir,
        path: 'src',
        annotate: ['state', 'events', 'attributes'],
        output: { max_tokens: 80 },
      })
    );
    expect(env.success).toBe(true);
