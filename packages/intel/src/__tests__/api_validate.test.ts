...
describe('api_validate', () => {
  ...
  it('errors cleanly when spec_path is missing', async () => {
    const env = expectError(
      await handler({ base_path: base } as unknown as Record<string, unknown>)
    );
  ...
  it('errors cleanly on an unparsable spec file', async () => {
    const env = expectError(
      await handler({ base_path: base, spec_path: 'package.json', framework: 'express' })
    );
