...
afterAll(async () => {
  if (prevStateRoot === undefined) {
    delete process.env.GOODVIBES_STATE_ROOT;
  } else {
    process.env.GOODVIBES_STATE_ROOT = prevStateRoot;
  }
...
describe('structural_edit: preview/apply round trip (exact mode)', () => {
  it('previews a diff + token without writing, then applies it', async () => {
    ...
    const preview = expectSuccess<{
      ...
      entries: Record<
        string,
        { status: string; diff?: string; match_count: number; resolved_path: string }
      >;
  ...
  it('a token is single-use: a second apply is refused', async () => {
    ...
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ path: 'c.ts', find: 'x', replace: 'y' }],
      })
    );
    const token = preview.data!.preview_token;
  ...
  it('keeps a no-op replacement byte-identical without rewriting the file', async () => {
    const file = await writeFile(dir, 'noop.ts', 'const value = 1;\n');
    ...
    const applied = expectSuccess<{ files_written: string[] }>(
      await call({ action: 'apply', preview_token: preview.data!.preview_token })
    );
    expect(applied.data!.files_written).toEqual([]);
  ...
  it('does not truncate when sequential edits cancel back to the original content', async () => {
    const file = await writeFile(dir, 'cancel.ts', 'x\n');
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [
          { path: 'cancel.ts', find: 'x', replace: 'y' },
          { path: 'cancel.ts', find: 'y', replace: 'x' },
        ],
      })
    );
    const applied = expectSuccess<{ files_written: string[] }>(
...
describe('structural_edit: ast mode (TypeScript compiler)', () => {
  it('replaces a whole function declaration node', async () => {
    ...
    const preview = expectSuccess<{
      preview_token: string;
      entries: Record<string, { status: string; match_count: number }>;
    }>(
      await call({
        action: 'preview',
        base_path: dir,
        ...
        edits: [
          {
            ...
            path: 'fn.ts',
            find: 'foo',
            replace: 'export function foo(): number {\n  return 2;\n}',
          },
        ],
      })
    );
...
describe('structural_edit: stale-hash refusal (never silently re-matches)', () => {
  it('refuses an entry whose file changed since preview', async () => {
    ...
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ id: 'e', path: 's.ts', find: 'value = 1', replace: 'value = 2' }],
      })
    );
  ...
  it('rechecks the hash immediately before replacement', async () => {
    const file = await writeFile(dir, 'late.ts', 'value = 1\n');
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ path: 'late.ts', find: 'value = 1', replace: 'value = 2' }],
      })
    );

    setBeforeReplaceHookForTests(async resolved => {
      if (resolved === file) {
        await fs.writeFile(file, 'value = external\n', 'utf8');
      }
    });
    try {
      ...
      expect(applied.data!.entries['0'].status).toBe('refused_stale');
      expect(applied.error).toMatch(/failed during a write/i);
      expect(await fs.readFile(file, 'utf8')).toBe('value = external\n');
    } finally {
      setBeforeReplaceHookForTests();
    }
  });
});
...
describe('structural_edit: atomic preflight and serialization', () => {
  it('does not touch a fresh file when another entry is stale', async () => {
    ...
    const beforeA = await fs.stat(fileA);
    // B changes underfoot; A does not.
    await fs.writeFile(fileB, 'BBB changed\n', 'utf-8');

    const apply = parseResult<{
      entries: Record<string, { status: string }>;
      summary: { applied: number; refused_stale: number; rolled_back: number; failed: number };
    }>(await call({ action: 'apply', preview_token: preview.data!.preview_token }));

    expect(apply.success).toBe(false);
    expect(apply.data!.entries.a.status).toBe('failed');
    expect(apply.data!.entries.b.status).toBe('refused_stale');
    expect(apply.data!.summary.rolled_back).toBe(0);
    expect(apply.data!.summary.failed).toBe(1);
    expect(apply.data!.summary.refused_stale).toBe(1);
    ...
    const afterA = await fs.stat(fileA);
    expect(afterA.ino).toBe(beforeA.ino);
    expect(afterA.mtimeMs).toBe(beforeA.mtimeMs);
  });

  it('serializes different tokens so only one can apply against the same original hash', async () => {
    const file = await writeFile(dir, 'race.ts', 'value = 1\n');
    const first = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ path: 'race.ts', find: 'value = 1', replace: 'value = 2' }],
      })
    );
    const second = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ path: 'race.ts', find: 'value = 1', replace: 'value = 3' }],
      })
    );

    const outcomes = await Promise.all([
      call({ action: 'apply', preview_token: first.data!.preview_token }),
      call({ action: 'apply', preview_token: second.data!.preview_token }),
    ]);
    const parsed = outcomes.map(outcome => parseResult(outcome));
    expect(parsed.filter(outcome => outcome.success)).toHaveLength(1);
    expect(parsed.filter(outcome => !outcome.success)).toHaveLength(1);
    expect(['value = 2\n', 'value = 3\n']).toContain(await fs.readFile(file, 'utf-8'));
  });
  ...
  it('partial mode applies the fresh entry and reports the stale one, without rollback', async () => {
    ...
    const apply = parseResult<{
      entries: Record<string, { status: string }>;
      summary: { applied: number; refused_stale: number };
    }>(await call({ action: 'apply', preview_token: preview.data!.preview_token }));
  ...
  it('leaves CRLF bytes outside the edit span exactly, and renders the replacement in CRLF too', async () => {
    ...
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        ...
        edits: [
          { path: 'crlf.ts', find: 'const b = 2;', replace: 'const b = 20;\nconst bb = 22;' },
        ],
      })
    );
    expectSuccess(await call({ action: 'apply', preview_token: preview.data!.preview_token }));
  ...
  it('rejects an expired token', async () => {
    ...
    const preview = expectSuccess<{ preview_token: string }>(
      await call({
        action: 'preview',
        base_path: dir,
        edits: [{ path: 'exp.ts', find: 'k = 1', replace: 'k = 2' }],
      })
    );
    const token = preview.data!.preview_token;
...
describe('structural_edit: ast_pattern behavior tracks @ast-grep/napi availability', () => {
  ...
  it.runIf(!astGrepAvailable)(
    'degrades honestly with an "unavailable" error, not a crash, when absent',
    async () => {
      await writeFile(dir, 'g.ts', 'console.log(1);\n');
      const preview = expectSuccess<{
        entries: Record<string, { status: string; error: string | null }>;
        summary: { error: number; ready: number };
      }>(
        await call({
          action: 'preview',
          base_path: dir,
          ...
          edits: [
            { id: 'p', path: 'g.ts', find: 'console.log($$$A)', replace: 'logger.info($$$A)' },
          ],
        })
      );
  ...
  it.runIf(astGrepAvailable)(
    'matches and rewrites structurally when installed (full round trip)',
    async () => {
      await writeFile(dir, 'g.ts', 'console.log(1);\nconsole.log(a, b);\nother(2);\n');
      const preview = expectSuccess<{
        preview_token: string;
  ...
  it('rejects a fuzzy/regex mode (only exact/ast/ast_pattern ship)', async () => {
    ...
    const r = parseResult(
      await call({
        ...
        base_path: dir,
        match: { mode: 'fuzzy' },
        edits: [{ path: 'm.ts', find: 'x', replace: 'y' }],
      })
    );
    expect(r.success).toBe(false);
