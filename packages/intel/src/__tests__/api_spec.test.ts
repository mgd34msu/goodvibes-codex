...
describe('api_spec', () => {
  it('generates an OpenAPI 3.0.3 spec from Express routes, read-only', async () => {
    ...
    const env = expectSuccess<ApiSpecData>(
      await handler({ base_path: base, framework: 'express' })
    );
    const data = env.data!;
  ...
  it('includes examples by default and omits them when include_examples: false', async () => {
    ...
    const withExamples = expectSuccess<ApiSpecData>(
      await handler({ base_path: base, framework: 'fastify' })
    );
    const anyOpWith =
      Object.values(withExamples.data!.spec.paths)[0]!.get ??
      Object.values(withExamples.data!.spec.paths)[0]!.post;
    ...
    const anyOpWithout =
      Object.values(withoutExamples.data!.spec.paths)[0]!.get ??
      Object.values(withoutExamples.data!.spec.paths)[0]!.post;
  ...
  it('emits YAML text alongside the JSON spec when format: "yaml"', async () => {
    ...
    const env = expectSuccess<ApiSpecData>(
      await handler({ base_path: base, framework: 'hono', format: 'yaml' })
    );
