...
describe('scaffold: dry_run', () => {
  it('previews vite-react without touching disk or a shell', async () => {
    ...
    expect(
      fs.existsSync(
        path.join('/tmp/goodvibes-scaffold-dry-run-does-not-exist', 'nonexistent-dry-run-dir')
      )
    ).toBe(false);
  ...
  it('defaults to preview-only with no package install or git initialization', async () => {
    const result = await scaffoldTool.handler({
      template: 'vite-react',
      output_dir: 'safe-default-preview',
      base_path: '/tmp/goodvibes-scaffold-dry-run-does-not-exist',
    });
    const envelope = JSON.parse((result.content[0] as { text: string }).text);
    expect(envelope.data.dry_run).toBe(true);
    expect(envelope.data.post_create_results).toEqual([]);
    expect(
      fs.existsSync(
        path.join('/tmp/goodvibes-scaffold-dry-run-does-not-exist', 'safe-default-preview')
      )
    ).toBe(false);
  });
