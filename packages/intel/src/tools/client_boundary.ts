...
import { collectScannableFiles, scanForDirectives } from '../frontend/client-boundary/scanner.js';
import {
  buildImportGraph,
  classifyComponents,
  buildBoundaryMap,
  ...
  inputSchema: {
    ...
    properties: {
      ...
      path: {
        type: 'string',
        description:
          'Directory to scan (relative to base_path or absolute). Auto-detects app/ then src/app then src/ then root.',
      },
      entry: {
        type: 'string',
      ...
      output: {
        ...
        properties: {
          max_tokens: {
            type: 'number',
            description:
              'Cap the rendered response; issues then components then boundaries trim to fit.',
          },
        },
      },
    },
...
export async function handler(rawArgs: unknown): Promise<CallToolResult> {
  ...
  try {
    const outcome = await withBudget(cfg.budgets.analyzer_ms, async () => {
      ...
      const sourceFiles = getSourceFiles(absPaths, baseDir);
      ...
      const summary: BoundarySummary = {
        ...
        clientInherited: classifications.filter(c => c.classification === 'client-inherited')
          .length,
      ...
      const boundaries: BoundaryEntry[] = Array.from(boundaryMap.entries())
        .map(([file, childCount]) => ({
          file,
          resolved_path: path.resolve(baseDir, file),
          childCount,
        }))
