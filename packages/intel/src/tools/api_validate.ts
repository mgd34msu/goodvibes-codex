  ...
  inputSchema: {
    ...
    properties: {
      ...
      spec_path: {
        type: 'string',
        description:
          'Path to the OpenAPI spec file (JSON or YAML), relative to base_path or absolute.',
      },
      path: {
        type: 'string',
        description:
...
export async function handler(rawArgs: unknown): Promise<CallToolResult> {
  ...
  try {
    ...
    try {
      ...
      if (ext === '.yaml' || ext === '.yml') {
        ...
        return toCallToolResult(
          errorEnvelope(`Unsupported spec file format: ${ext}. Use .json, .yaml, or .yml.`)
        );
      }
    } catch (parseError) {
    ...
    if (!spec || typeof spec !== 'object' || !spec.paths || typeof spec.paths !== 'object') {
      return toCallToolResult(
        errorEnvelope('Spec file has no "paths" object; not a valid OpenAPI/Swagger document.')
      );
    }
    ...
    const specEndpointsCount = Object.values(spec.paths).reduce((sum, item) => {
      ...
      return (
        sum +
        ['get', 'post', 'put', 'patch', 'delete', 'head', 'options'].filter(m => rec[m]).length
      );
    }, 0);
