...
import {
  ...
  type OutlineNode as TSOutlineNode,
} from '../lib/tree-sitter.js';
...
async function readSingleFile(
  ...
  if (spec.probe) {
    ...
    if (lookup.previousReadAt !== undefined) {
    ...
    if (lookup.status === 'modified') {
      ...
      if (lookup.changes) {
        result.cache.changes = lookup.changes;
      }
      if (lookup.modifiedBy) {
        result.cache.modified_by = lookup.modifiedBy;
      }
    }
    return result;
  }
  ...
  if (
    ...
    !spec.force &&
    !hasRange &&
    (stats.size > MAX_FILE_BYTES || estimatedTokens > MAX_TOKEN_ESTIMATE)
  ) {
    ...
    const completeLines =
      readWholeFile || allPartialLines.length <= 1 ? allPartialLines : allPartialLines.slice(0, -1);
  ...
  if (extract === 'lines') {
    ...
    if (!isLanguageSupported(resolved_path)) {
      ...
      try {
        ...
        // the standard maintenance-pointer message; any other parse failure keeps its
...
function paginateSingleResult(
  ...
  page: FileReadResult;
  meta: { page: number; total_pages: number; tokens_used: number; warning?: string };
} {
  const field: 'lines' | 'outline' | null = result.lines
    ? 'lines'
    : result.outline
      ? 'outline'
      : null;
  ...
  const items =
    ((result as unknown as Record<string, unknown[]>)[field] as unknown[] | undefined) ?? [];
  const overheadEstimate =
    tokenBudget < 300 ? Math.floor(tokenBudget * 0.7) : Math.floor(tokenBudget * 0.6);
  ...
  for (const item of items) {
    const itemTokens = estimatePayloadTokens(
  ...
  return {
    page: pageResult,
    meta: { page: pageIndex + 1, total_pages: totalPages, tokens_used: tokensUsed, warning },
  };
}
...
function paginateByTokenBudget(
  ...
  return {
    ...
    meta: {
      ...
      budget_exceeded: budgetExceeded || undefined,
      warning,
    },
  };
}
  ...
  inputSchema: {
    ...
    properties: {
      ...
      base_path: {
        type: 'string',
        description:
          'Root directory files resolve against; omitting it falls back to the server cwd with a warning.',
      },
      default_range: {
        type: 'object',
        properties: { start: { type: 'number' }, end: { type: 'number' } },
      ...
      token_budget: {
        type: 'number',
        description: 'Bin-pack results across pages within this token budget',
      },
      page: { type: 'number', description: '1-indexed page to return (requires token_budget)' },
      force: {
        type: 'boolean',
        description: 'Bypass cache freshness metadata and the size gate for every file',
      },
      probe: {
        type: 'boolean',
        description: 'Freshness probe: metadata only, no content, for every file',
      },
      output: {
        type: 'object',
        properties: {
...
async function runCodeRead(args: unknown): Promise<CallToolResult> {
  ...
  try {
    ...
    if (files.length === 0) {
      return toCallToolResult(
        errorEnvelope(
          "Missing required parameter 'files'. Expected: array of file paths or file specs.",
          {
            execution_ms: Math.round(performance.now() - start),
          }
        )
    ...
    const forceRead =
      (input.token_budget !== undefined && input.token_budget > 0) || input.force === true;
    ...
    if (
      ...
      input.page > 1 &&
      (input.token_budget === undefined || input.token_budget <= 0)
    ) {
    ...
    const buildEntry = (r: FileReadResult, verbose: boolean): Record<string, unknown> => {
      ...
      if (verbose && r.token_cost !== undefined) {
    ...
    switch (output.mode) {
      ...
      case 'verbose':
        data = {
          files: Object.fromEntries(
            paginatedResults.map(r => [r.result_key ?? r.path, buildEntry(r, true)])
          ),
          summary,
      ...
      default:
        data = {
          files: Object.fromEntries(
            paginatedResults.map(r => [r.result_key ?? r.path, buildEntry(r, false)])
          ),
          summary,
          tokens_used: estimatePayloadTokens(JSON.stringify(paginatedResults)),
        };
      ...
      while (estimateRendered() > maxTokens) {
        ...
        for (const r of paginatedResults) {
          if (
            r.outline &&
            r.outline.length > 0 &&
            (!largest || r.outline.length > largest.length)
          ) {
            largest = r.outline;
            owner = r;
          }
