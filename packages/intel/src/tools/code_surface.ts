      ...
      output: {
        ...
        properties: {
          max_tokens: {
            type: 'number',
            description:
              'Cap the rendered response; the surface lists trim to fit (internal first).',
          },
        },
      },
    },
...
async function computeSurface(
  ...
  if (explicitEntryPoints && explicitEntryPoints.length > 0) {
    ...
    entryPoints = await detectEntryPoints(absTarget, absTarget);
  }
  ...
  const sourceFiles = await findSourceFiles(absTarget, absTarget);
  ...
  const { service } = host.getServiceForFiles(allFiles, absTarget);
...
function capToTokens(
  env: Envelope<CodeSurfaceData>,
  maxTokens?: number
): Envelope<CodeSurfaceData> {
  ...
  while (
    data.internal_api.length > 0 &&
    estimatePayloadTokens(renderEnvelope(trim())) > maxTokens
  ) {
