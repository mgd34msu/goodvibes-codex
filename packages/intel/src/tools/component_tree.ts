...
import { analyzeFile, findComponentFiles } from '../frontend/react/component-analyzer.js';
import {
...
} from '../frontend/annotations/attributes.js';
  ...
  inputSchema: {
    ...
    properties: {
      ...
      path: {
        type: 'string',
        description:
          'File or directory to analyze (relative to base_path or absolute). Default "src".',
      },
      annotate: {
        type: 'array',
        items: { type: 'string', enum: ALL_MODES },
        description:
          'Annotation modes to attach: state, boundaries, events, attributes. Default [] (bare tree).',
      },
...
export async function handler(rawArgs: unknown): Promise<CallToolResult> {
  ...
  try {
    ...
    const outcome = await withBudget(cfg.budgets.analyzer_ms, async signal => {
      ...
      if (stat.isFile()) {
        const sf = getSourceFile(absTarget, baseDir);
        if (sf) {
          allComponents.push(
        ...
        const sourceFiles = getSourceFiles(files, baseDir);
        for (const abs of files) {
          ...
          allComponents.push(...analyzeFile(sf, makeRelativePath(abs, baseDir), abs, nodeIndex));
        }
      }
    ...
    const data: ComponentTreeData = {
      ...
      ...(outcome.value.count === 0
        ? { message: `No React components found in ${targetInput}` }
        : {}),
    };
