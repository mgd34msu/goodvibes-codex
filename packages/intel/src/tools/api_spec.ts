 ...
 *
 * Pairs with `api_routes`, calling the same `scanFrameworkRoutes` orchestration
 * directly in process rather than invoking a handler and re-parsing its JSON.
 *
 * READ-ONLY: the generated spec is returned in the response and is never written
 * to disk. `format: "yaml"` returns the YAML text in memory alongside the JSON
 * `spec` object.
 *
  ...
  inputSchema: {
    ...
    properties: {
      ...
      path: {
        type: 'string',
        description:
          'Directory to scan for routes (relative to base_path or absolute). Default ".".',
      },
      framework: {
        type: 'string',
      ...
      format: {
        ...
        enum: ['json', 'yaml'],
        description:
          "Also render 'yaml' text in the response alongside the JSON spec object (default 'json', no extra text).",
      },
    },
  },
};

/** Build the OpenAPI spec from routes plus package.json defaults. */
function buildSpec(
  routes: ApiRoute[],
  args: ApiSpecArgs,
  ...
  const spec: OpenAPISpec = {
    ...
    info: {
      ...
      ...((args.description || pkg.description) && {
  ...
  for (const openApiPath of Object.keys(routesByPath)) {
    ...
    for (const route of routesByPath[openApiPath]) {
      ...
      const { requestSchema, responseSchema } = parseHandlerTypes(
        path.resolve(baseDir, route.handler_file)
      );

      const hasRequestSchema =
        requestSchema !== null && Object.keys(requestSchema.properties || {}).length > 0;
      const hasResponseSchema =
        responseSchema !== null && Object.keys(responseSchema.properties || {}).length > 0;
