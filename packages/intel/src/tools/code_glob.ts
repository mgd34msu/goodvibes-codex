...
import { assertExistingPathWithinSelectedRoot, loadGitignorePatterns } from '@goodvibes/core/fsx';
...
/** Glob engines accept parent segments and absolute patterns; neither is valid inside a selected workspace. */
function assertSafeGlobPattern(pattern: string): void {
  if (
    ...
    throw new Error(
      `Glob pattern '${pattern}' may not escape or replace the selected workspace root.`
    );
  }
}
...
async function listCandidateFiles(
  ...
  if (backend === 'fast-glob') {
    ...
    if (fg) {
      if (followSymlinks) {
        // fast-glob follows links while walking, before returned matches can be
        // checked. Audit the same non-ignored tree without following links
        // first, so an escaping directory link is rejected before traversal.
        const boundaryEntries = await fg(['**/*'], {
          cwd: workDir,
        ...
        for (const entry of boundaryEntries) {
          ...
          assertExistingPathWithinSelectedRoot(entryPath, workDir);
        }
      }
      ...
      return {
        files: raw.map(f =>
          typeof f === 'string'
            ? { path: f, stats: null }
            : { path: f.path, stats: f.stats ?? null }
        ),
      };
    }
  ...
  inputSchema: {
    ...
    properties: {
      patterns: {
        type: 'array',
        items: { type: 'string' },
        description: 'Glob patterns, e.g. ["**/*.ts"]',
      },
      patterns_base64: {
        type: 'array',
        items: { type: 'string' },
        description: 'Base64-encoded patterns (for patterns containing brackets etc.)',
      },
      preset: {
        type: 'string',
        enum: Object.keys(GLOB_PRESETS),
        description: 'Named pattern preset, used when patterns/patterns_base64 are omitted',
      },
      base_path: {
        type: 'string',
        description:
          'Root directory to search from. Relative patterns resolve against it; omitting it falls back to the server cwd with a warning.',
      },
      exclude: {
        type: 'array',
        items: { type: 'string' },
        description: 'Additional glob excludes',
      },
      respect_gitignore: {
        type: 'boolean',
        default: true,
        description: 'Apply DEFAULT_EXCLUDES and the root .gitignore of base_path',
      },
      ...
      filters: {
        ...
        properties: {
          ...
          has_content: {
            type: 'string',
            description: 'Pattern that file content must match (ripgrep-searched)',
          },
...
async function runCodeGlob(args: unknown, _signal: { aborted: boolean }): Promise<CallToolResult> {
  ...
  try {
    ...
    let files: Array<{ path: string; stats: Stats | null }> = [];
    for (const file of listed.files) {
      const candidate = path.isAbsolute(file.path) ? file.path : path.resolve(workDir, file.path);
      const canonical = assertExistingPathWithinSelectedRoot(candidate, workDir);
      try {
        files.push({ path: canonical, stats: await fs.stat(canonical) });
      } catch {
        // A file can disappear between listing and stat; omit that stale result.
      }
    }
        ...
        matchingFiles.map(f =>
          path.normalize(
            assertExistingPathWithinSelectedRoot(
              path.isAbsolute(f) ? f : path.resolve(workDir, f),
              workDir
            )
          )
        )
    ...
    for (const file of files) {
      ...
      const absolutePath = assertExistingPathWithinSelectedRoot(
        path.isAbsolute(file.path) ? file.path : path.resolve(workDir, file.path),
        workDir
      );
    ...
    if (truncatedByTokens && rawOutput.max_tokens !== undefined) {
    ...
    switch (mode) {
      ...
      case 'with_stats':
        data = {
          files: results.map(r => ({ path: r.path, resolved_path: r.resolved_path, ...r.stats })),
          summary,
