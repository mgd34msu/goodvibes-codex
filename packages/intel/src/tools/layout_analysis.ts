  ...
  inputSchema: {
    ...
    properties: {
      ...
      sections: {
        ...
        description:
          'Sections to compute. Default ["overflow","stacking"]. "sizing" requires selector; "responsive" is post-alpha.',
      },
      output: {
        type: 'object',
        properties: {
          max_tokens: {
            type: 'number',
            description:
              'Cap the rendered response; stacking contexts, overflow risks, then hierarchy leaves trim to fit.',
          },
        },
      },
    },
...
export async function handler(rawArgs: unknown): Promise<CallToolResult> {
  ...
  const requested: Section[] = Array.isArray(args.sections)
    ? args.sections.filter((s): s is Section =>
        ['overflow', 'sizing', 'stacking', 'responsive'].includes(s)
      )
    : DEFAULT_SECTIONS;
  const selector =
  ...
  if (!(FRONTEND_EXTENSIONS as readonly string[]).includes(ext)) {
    return toCallToolResult(
      errorEnvelope(
        `Unsupported file type: ${ext || '(none)'}. Supported: ${FRONTEND_EXTENSIONS.join(', ')}.`
      )
    );
  }
  ...
  try {
    ...
    const outcome = await withBudget(cfg.budgets.analyzer_ms, async () => {
      const sourceFile = getSourceFile(absFile, baseDir);
      if (!sourceFile) {
        return { ok: false as const, error: `Failed to parse component file: ${absFile}` };
      }
      ...
      if (!tree) {
        return { ok: false as const, error: 'Failed to parse layout hierarchy from JSX.' };
      }
      ...
      if (requested.includes('sizing')) {
        if (!selector) {
          data.sizing = {
            selector: null,
            note: 'The sizing section requires a `selector` to focus on one node.',
          };
        } else {
          ...
          if (!el) {
            ...
            data.sizing = {
              ...
              constraint_chain: buildAncestorChain(el).map(a => ({
                ancestor: a.element,
                constraint: a.sizing_impact,
              })),
            };
          }
        }
