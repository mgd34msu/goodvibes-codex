...
async function transformRipgrepResult(
  ...
  for (const match of ripgrepResult.matches) {
    const candidate = path.isAbsolute(match.file) ? match.file : path.resolve(workDir, match.file);
    const canonical = assertExistingPathWithinSelectedRoot(candidate, workDir);
    const relativePath = path.relative(workDir, canonical);
    const existing = byFile.get(relativePath);
    const safeMatch = { ...match, file: canonical };
    if (existing) {
      existing.push(safeMatch);
    } else {
      byFile.set(relativePath, [safeMatch]);
    }
  }
  ...
  const includedEntries =
    fileEntries.length > maxFiles ? fileEntries.slice(0, maxFiles) : fileEntries;
  if (includedEntries.length < fileEntries.length) {
    truncated = true;
    effectiveCaps.max_results = maxFiles;
  }

  const resolvedPathOf = (relativePath: string): string =>
    assertExistingPathWithinSelectedRoot(path.resolve(workDir, relativePath), workDir);
    ...
    for (const match of fileMatches) {
      ...
      if (output.mode === 'context') {
        if (output.expand_to === 'function' || output.expand_to === 'class') {
          try {
            const fileContent = await fs.readFile(resolvedPathOf(relativePath), 'utf-8');
  ...
  result.tokens_used =
    totalTokens > 0 ? totalTokens : estimatePayloadTokens(JSON.stringify(result));
  return result;
}
...
async function executeQuery(
  query: GrepQuery,
  output: Required<
    Pick<GrepOutput, 'mode' | 'max_results' | 'max_per_item' | 'max_total_matches'>
  > &
    GrepOutput,
  workDir: string
): Promise<GrepResult> {
  ...
  if (
    output.expand_to &&
    output.context_before === undefined &&
    output.context_after === undefined
  ) {
  ...
  if (!patternStr) {
    throw new Error(
      "Missing required parameter 'queries[].pattern' (or 'queries[].pattern_base64')."
    );
  }

  if (query.negate === true) {
    const searchPath = query.path
      ? assertExistingPathWithinSelectedRoot(path.resolve(workDir, query.path), workDir)
      : workDir;
    ...
    negationResult.files = negationResult.files.map(file => {
      const resolvedPath = assertExistingPathWithinSelectedRoot(file.resolved_path, workDir);
      return { file: path.relative(workDir, resolvedPath), resolved_path: resolvedPath };
    });
    ...
    const negationReturn: GrepResult = {
      ...
      negation: negationResult,
      tokens_used: estimatePayloadTokens(
        JSON.stringify({ files: negationResult.files, negation: negationResult })
      ),
    };
  ...
  if (query.glob && !query.path) {
    ...
    if (split) {
      searchPath = assertExistingPathWithinSelectedRoot(path.resolve(workDir, split.dir), workDir);
    ...
    const requestedPath = path.isAbsolute(query.path)
      ? query.path
      : path.resolve(workDir, query.path);
    let absolutePath: string;
    try {
      absolutePath = assertExistingPathWithinSelectedRoot(requestedPath, workDir);
    } catch (error) {
  ...
  inputSchema: {
    ...
    properties: {
      queries: {
        ...
        items: {
          ...
          properties: {
            ...
            path: {
              type: 'string',
              description: 'File or directory to search (relative to base_path)',
            },
            ...
            negate: {
              type: 'boolean',
              default: false,
              description: 'Find files WITHOUT the pattern',
            },
      ...
      base_path: {
        type: 'string',
        description:
          'Root directory queries resolve against; omitting it falls back to the server cwd with a warning.',
      },
      parallel: { type: 'boolean', default: true },
      ranked: {
        type: 'boolean',
        default: false,
        description: "Sort each query's files by a cheap relevance score",
      },
      preview_replace: {
        type: 'string',
        description: 'Dry-run replacement string; requires matches mode/content',
      },
      output: {
        type: 'object',
        properties: {
          ...
          max_per_item: {
            type: 'number',
            default: 10,
            description: 'Caps matches included PER FILE',
          },
          max_total_matches: {
            type: 'number',
            default: 100,
            description: 'Caps matches included ACROSS ALL FILES',
          },
  ...
  try {
    ...
    for (const query of queries) {
      if (!query.id) {
        return toCallToolResult(
          errorEnvelope(
            "Missing required parameter 'queries[].id'. Expected: string identifier for each query.",
            {
              execution_ms: Math.round(performance.now() - start),
            }
          )
      ...
      if (!query.pattern && !query.pattern_base64) {
        return toCallToolResult(
          errorEnvelope(
            "Missing required parameter 'queries[].pattern'. Expected: search pattern (string or base64).",
            {
              execution_ms: Math.round(performance.now() - start),
            }
          )
    ...
    for (const [queryId, result] of Object.entries(queryResults)) {
      ...
      const patternStr =
        resolveStringOrBase64(query as unknown as Record<string, unknown>, 'pattern') ?? '';
      ...
      try {
        ...
        result.note =
          (result.note ? result.note + '; ' : '') + `Pagination failed: ${(err as Error).message}`;
      }
      ...
      try {
        ...
        result.note =
          (result.note ? result.note + '; ' : '') + `Ranking failed: ${(err as Error).message}`;
      }
    ...
    const env: Envelope<unknown> = successEnvelope(data, {
      execution_ms: Math.round(performance.now() - start),
    });
    if (baseWarning) {
    ...
    return toCallToolResult(
      errorEnvelope((error as Error).message, {
        execution_ms: Math.round(performance.now() - start),
      })
    );
  }
