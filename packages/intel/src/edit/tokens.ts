...
import { goodvibesDataRoot } from '@goodvibes/core/workspace';
...
function tokenDir(): string {
  return process.env.GOODVIBES_STATE_ROOT
    ? path.join(process.env.GOODVIBES_STATE_ROOT, '.goodvibes', 'codex', 'edit-tokens')
    : path.join(goodvibesDataRoot(), 'edit-tokens');
}
...
export function newTokenId(): string {
  return `se_${Date.now().toString(36)}_${crypto.randomBytes(16).toString('hex')}`;
}
...
export async function saveToken(tok: PreviewToken): Promise<void> {
  ...
  const destination = path.join(dir, `${tok.token}.json`);
  const temporary = `${destination}.${process.pid}.${crypto.randomBytes(6).toString('hex')}.tmp`;
  await fs.writeFile(temporary, JSON.stringify(tok), {
    encoding: 'utf-8',
    mode: 0o600,
    flag: 'wx',
  });
...
export async function loadAndConsumeToken(token: string): Promise<PreviewToken | null> {
  ...
  const claimed = path.join(
    tokenDir(),
    `.${token}.${process.pid}.${crypto.randomBytes(6).toString('hex')}.consuming`
  );
  let raw: string;
  try {
    // Atomic rename is the single-use claim. Concurrent consumers cannot both
    // rename the same source path, so only one can ever observe the record.
    await fs.rename(file, claimed);
    raw = await fs.readFile(claimed, 'utf-8');
  } catch {
    return null;
  }
  await fs.unlink(claimed).catch(() => {});
  try {
    const parsed = JSON.parse(raw) as PreviewToken;
