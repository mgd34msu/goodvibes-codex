...
function astMatchSpans(filePath: string, original: string, find: string): Span[] {
  ...
  const matchesPattern = (node: ts.Node): boolean => {
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
      ...
      return (
        pattern === `function ${name}` || pattern === `async function ${name}` || pattern === name
      );
    }
...
async function astPatternSpans(
  ...
  if (!langName) {
    return {
      spans: [],
      available: true,
      reason: `ast_pattern: unsupported language for '${filePath}'.`,
    };
  }
  try {
    ...
    return {
      spans: [],
      available: true,
      reason: `ast_pattern matching failed: ${(err as Error).message}`,
    };
  }
}
...
function applySpans(original: string, spans: Span[], replace: string): string {
  ...
  for (const span of [...spans].sort((a, b) => b.start - a.start)) {
    const replacement =
      span.replacement !== undefined ? render(span.replacement) : defaultReplacement;
...
export async function computeEdit(content: string, req: EditRequest): Promise<ComputedEdit> {
  ...
  if (req.mode === 'ast') {
    ...
    const result = await astPatternSpans(
      req.filePath,
      content,
      req.find,
      req.replace,
