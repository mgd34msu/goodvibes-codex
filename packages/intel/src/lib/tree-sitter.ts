...
export class TreeSitterCore {
   ...
   *   installed yet, callers surface the standard maintenance-pointer envelope.
   */
  ...
  async parse(content: string, filePath: string): Promise<Tree> {
    ...
    if (this.currentLanguage !== langName) {
      ...
      if (!lang) {
        throw new Error(
          `Language not available: ${langName}. ${this.lastLoadError ?? 'WASM grammar not found.'}`
        );
      }
