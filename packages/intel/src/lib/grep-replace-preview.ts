...
function generateUnifiedDiffHunk(
  file: string,
  line: number,
  before: string,
  after: string
  ...
  if (highlight && highlight.length === 2) {
    ...
    if (start < 0 || end > content.length || start >= end) {
      try {
        ...
        return [
          content.replace(regex, replaceString),
          'Invalid highlight range, used regex fallback',
        ];
      } catch {
        return [
          content.replace(searchPattern, replaceString),
          'Invalid highlight range, used literal replacement fallback',
        ];
      }
    }
    ...
    try {
      ...
      return [
        before + replaceString + after,
        'Regex compilation failed, used literal replacement fallback',
      ];
    }
  }
  try {
    ...
    try {
      ...
      return [
        content.replace(regex, replaceString),
        'Regex compilation failed, used escaped pattern fallback',
      ];
    } catch {
      return [
        content.split(searchPattern).join(replaceString),
        'Regex compilation failed, used simple string replacement fallback',
      ];
    }
  }
}
