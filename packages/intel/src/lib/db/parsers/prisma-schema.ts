  ...
  while ((match = modelRegex.exec(content)) !== null) {
    ...
    for (const line of modelBody.split('\n')) {
      ...
      if (fieldMatch) {
        ...
        if (isRelation) {
          const relationMatch = cleanRest.match(
            /@relation\s*\([^)]*fields:\s*\[([^\]]+)\][^)]*references:\s*\[([^\]]+)\]/
          );
          if (relationMatch) {
          ...
          if (refMatch) {
            ...
            const relationField = modelBody.match(
              new RegExp(`(\\w+)\\s+\\w+.*@relation.*fields:\\s*\\[${fieldName}\\]`)
            );
            if (relationField) {
    ...
    while ((idxMatch = indexRegex.exec(modelBody)) !== null) {
      ...
      const idxName =
        idxMatch[3] || `${modelName}_${idxColumns.join('_')}_${isUnique ? 'unique' : 'idx'}`;
