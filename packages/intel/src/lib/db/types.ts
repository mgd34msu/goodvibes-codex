 ...
 *
 * `DatabaseTable`, `DatabaseRelation` and `DatabaseColumn` are the unified
 * parser output: flat tables plus a global relations array. All three schema
 * parsers (Prisma, Drizzle, SQL) produce this shape. The tool boundary
 * (`tools/db_schema.ts`) reshapes it into the `models[].relations` shape on the
 * way out.
 *
 * @module lib/db/types
 */
...
/** Parser output: the unified schema extraction result. */
...
// =============================================================================
// db_schema tool shape
// =============================================================================

/** A field on a `db_schema` model, same content as DatabaseColumn. */
...
/** A relation embedded on its owning model, not a flat global array. */
...
/** One model in the `db_schema` output shape. */
