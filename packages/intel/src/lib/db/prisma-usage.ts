...
function extractModelFromPrismaCall(
  ...
  const isValidClient = validClients.some(
    c => clientText === c || clientText.endsWith('.' + c.split('.').pop())
  );
  if (!isValidClient) {
    return null;
  }
...
export async function scanPrismaUsage(
  ...
  const allFiles = (await findSourceFiles(baseDir, baseDir)).filter(
  ...
  const { program } = host.getServiceForFiles(candidateFiles.map(toTsPath), baseDir);
