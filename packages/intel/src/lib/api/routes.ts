...
async function existsDir(p: string, analysisRoot: string): Promise<boolean> {
  try {
    const safePath = assertExistingPathWithinSelectedRoot(p, analysisRoot);
...
async function scanGenericFramework(
  ...
  const searchDirs = (await existsDir(srcDir, absTarget)) ? [srcDir] : [absTarget];

  const routes: ApiRoute[] = [];
  for (const dir of searchDirs) {
    const files = (await findSourceFiles(dir, absTarget)).filter(
      f => SCAN_EXT.test(f) && !f.endsWith('.d.ts')
    );
...
async function scanNextJs(
  ...
  for (const dir of [
    path.join(absTarget, 'src', 'app', 'api'),
    path.join(absTarget, 'app', 'api'),
  ]) {
    if (!(await existsDir(dir, absTarget))) {
      continue;
    }
    const files = (await findSourceFiles(dir, absTarget)).filter(f =>
      /route\.(ts|tsx|js|jsx)$/.test(f)
    );
    for (const file of files) {
      if (signal?.aborted) {
        return routes;
  ...
  for (const dir of [
    path.join(absTarget, 'src', 'pages', 'api'),
    path.join(absTarget, 'pages', 'api'),
  ]) {
    if (!(await existsDir(dir, absTarget))) {
      continue;
    }
