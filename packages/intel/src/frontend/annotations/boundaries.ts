...
function findClass(node: ts.Node): ts.ClassDeclaration | null {
  ...
  ts.forEachChild(node, child => {
    if (!found && ts.isClassDeclaration(child)) {
      found = child;
    }
  });
  return found;
 ...
 */
export function annotateBoundaries(
  componentNode: ts.Node,
  sourceFile: ts.SourceFile
): BoundaryAnnotation | null {
  ...
  for (const [importedName, moduleName] of imports) {
    ...
    if (moduleName === 'react-error-boundary') {
      if (
        (REACT_ERROR_BOUNDARY_COMPONENTS.has(localName) || localName === 'ErrorBoundary') &&
        jsxNames.has(localName)
      ) {
        return {
          is_boundary: true,
          mechanism: localName,
          has_fallback: jsxHasProp(componentNode, sourceFile, localName, [
            'fallback',
            'FallbackComponent',
            'fallbackRender',
          ]),
          has_reset: jsxHasProp(componentNode, sourceFile, localName, [
            'onReset',
            'resetKeys',
            'onError',
          ]),
    ...
    if (LIBRARY_BOUNDARY_SOURCES.has(moduleName)) {
      ...
      return {
        ...
        has_fallback: jsxHasProp(componentNode, sourceFile, matched, [
          'fallback',
          'FallbackComponent',
          'fallbackRender',
        ]),
        has_reset: jsxHasProp(componentNode, sourceFile, matched, [
          'onReset',
          'resetKeys',
          'onError',
        ]),
