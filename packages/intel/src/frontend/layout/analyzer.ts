/**
 * Layout hierarchy analyzer (backbone).
 *
 * Trimmed to the tree backbone `layout_analysis` needs: a JSX to LayoutNode tree
 * built through the shared Tailwind class dictionary. There is no generic
 * issue-detector, constraint-notes or summary here, because overflow, sizing and
 * stacking are their own sections of the merged shape.
 *
    ...
    if (selector && !foundSelector && !elementMatches) {
      if (children.length > 0) {
        return children.length === 1
          ? children[0]
          : buildLayoutNode(tagName, classes, id, props, children);
      }
      return null;
    }
