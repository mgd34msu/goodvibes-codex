...
export function classifyDependency(
  ...
  if (depText.includes('=>') || depText.trimStart().startsWith('function')) {
    return {
      stability: 'unstable',
      reason:
        'Inline function expression — creates new reference every render; wrap in useCallback',
    };
  }
  ...
  if (scope.stateVars.has(base)) {
    ...
    return {
      stability: 'unknown',
      reason:
        'State variable — stable for primitives, unstable for object/array state (new ref on each setState)',
    };
  }
