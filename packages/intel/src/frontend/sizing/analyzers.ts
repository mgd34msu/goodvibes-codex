...
export function analyzeWidthStrategy(element: ElementNode): SizingDimension {
  ...
  while (current) {
    if (current.maxWidth) {
      constraints.push(
        `max-width: ${current.maxWidth} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
    if (current.width?.strategy === 'fixed' || current.width?.strategy === 'viewport') {
      constraints.push(
        `parent width: ${current.width.value} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
    if (current.overflowX === 'hidden' || current.overflowX === 'clip') {
      constraints.push(
        `overflow-x: ${current.overflowX} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
...
export function analyzeHeightStrategy(element: ElementNode): SizingDimension {
  ...
  while (current) {
    if (current.maxHeight) {
      constraints.push(
        `max-height: ${current.maxHeight} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
    if (current.height?.strategy === 'fixed' || current.height?.strategy === 'viewport') {
      constraints.push(
        `parent height: ${current.height.value} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
    if (current.overflowY === 'hidden' || current.overflowY === 'clip') {
      constraints.push(
        `overflow-y: ${current.overflowY} (from ${createElementIdentifier(current.tagName, current.classes, current.id)})`
      );
    }
    if (
      ...
      constraints.push(
        `WARNING: percentage height may not work (${createElementIdentifier(current.tagName, current.classes, current.id)} has auto height)`
      );
    }
    current = current.parent;
  }
...
export function analyzeFlexBehavior(element: ElementNode): FlexBehavior | undefined {
  const hasFlex =
    element.flexGrow !== undefined ||
    element.flexShrink !== undefined ||
    element.flexBasis !== undefined;
  const isFlexContainer = element.display === 'flex' || element.display === 'inline-flex';
  const parentIsFlexContainer =
    element.parent?.display === 'flex' || element.parent?.display === 'inline-flex';
...
export function analyzeGridBehavior(element: ElementNode): GridBehavior | undefined {
  const hasGrid =
    ...
    element.gridArea !== undefined;
  const isGridContainer = element.display === 'grid' || element.display === 'inline-grid';
  const parentIsGridContainer =
    element.parent?.display === 'grid' || element.parent?.display === 'inline-grid';
