...
export function extractTypeInfo(
  ...
  analysisRoot: string = path.dirname(absoluteFilePath)
): FileTypeInfo {
  const symbols: SymbolInfo[] = [];
  try {
    const { service, program } = host.getServiceForFile(absoluteFilePath, analysisRoot);
    ...
    for (const item of navTree?.childItems ?? []) {
      if (item.text.startsWith('<') || item.text.startsWith('_')) {
        continue;
      }
...
export async function extractTypeInfoFromContent(
  ...
  return withTempFile(tempFile, content, filePath =>
    extractTypeInfo(host, filePath, originalPath, projectRoot)
  );
}
