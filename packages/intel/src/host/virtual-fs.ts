...
import { assertPathWithinSelectedRoot } from '@goodvibes/core/fsx';
...
export function applyEdit(
  ...
  if (edit.old_text !== undefined && edit.new_text !== undefined) {
    ...
    if (occurrences > 1) {
      return [
        null,
        `old_text matches ${occurrences} locations. Provide more context to disambiguate.`,
      ];
    }
...
export async function createVirtualLanguageService(
  ...
  const configPath = await findTsConfig(projectRoot, projectRoot);
  const compilerOptions = configPath
    ? await readTsConfig(configPath, projectRoot)
    : { ...TS_ANALYSIS_OPTIONS };
  const tsLibDir =
    findTypescriptLibDir(projectRoot, projectRoot) ??
    path.dirname(ts.getDefaultLibFilePath(compilerOptions));
  const rootFiles = filesToCheck.map(fileName =>
    toTsPath(assertPathWithinSelectedRoot(fileName, projectRoot))
  );
  const allowedPath = (fileName: string): string | null => {
    const candidate = path.isAbsolute(fileName) ? fileName : path.resolve(projectRoot, fileName);
    try {
      return assertPathWithinSelectedRoot(candidate, projectRoot);
    } catch {
      try {
  ...
  const host: ts.LanguageServiceHost = {
    ...
    getScriptSnapshot: fileName => {
      ...
      const content = vfs.getContent(allowed);
    ...
    fileExists: fileName => {
      const allowed = allowedPath(fileName);
      return allowed ? vfs.exists(allowed) : false;
    },
    readFile: fileName => {
      const allowed = allowedPath(fileName);
      return allowed ? vfs.getContent(allowed) : undefined;
    },
    ...
    realpath: fileName => {
      const allowed = allowedPath(fileName);
      return allowed ? (ts.sys.realpath?.(allowed) ?? allowed) : fileName;
    },
  };
