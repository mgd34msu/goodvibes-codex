...
export async function findSourceFiles(
  ...
  async function walk(dir: string): Promise<void> {
    const safeDir = assertExistingPathWithinSelectedRoot(dir, root);
    if (visited.has(safeDir)) {
      return;
    }
    visited.add(safeDir);
    let entries;
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) {
        // Resolve every discovered symlink even when it would otherwise be
        // skipped. Escaping links are a boundary violation, not an absent file.
        const target = assertExistingPathWithinSelectedRoot(fullPath, root);
        const targetStat = await fsPromises.stat(target);
        if (targetStat.isFile() && isSourceFile(entry.name)) {
          files.push(target);
        }
        continue;
      }
      if (entry.isDirectory()) {
        ...
        files.push(assertExistingPathWithinSelectedRoot(fullPath, root));
      }
    }
  }
