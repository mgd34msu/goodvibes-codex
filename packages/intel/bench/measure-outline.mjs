...
async function main() {
  ...
  for (const r of rows) {
    if (r.error) {
      anyError = true;
      console.log(
        `| ${r.file} | ${r.lines} | ${r.rawBytes} | ${r.nativeTokens}t | ERROR: ${r.error} | — |`
      );
      continue;
    }
  ...
  if (anyError) {
    ...
    console.log(
      allBeatNative
        ? 'VERDICT: PASS — code_read outline beats native full read on every file.'
        : 'VERDICT: FAIL — code_read outline did not beat native full read on every file.'
    );
