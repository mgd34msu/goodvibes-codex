...
describe('secrets-store', () => {
  ...
  describe('getServiceSecrets / setServiceSecret', () => {
    ...
    it('should store and retrieve service auth', async () => {
      await store.setServiceSecret('myapi', {
        type: 'api-key',
        header: 'X-API-Key',
        key: 'secret123',
  ...
  describe('removeServiceSecret', () => {
    ...
    it('should remove an existing service', async () => {
      await store.setServiceSecret('temp', {
        type: 'bearer',
        token: 'x',
        service_origin: 'https://temp.example.test',
      });
  ...
  describe('resolveSecretValue', () => {
    ...
    it('does not resolve arbitrary inherited env var references', () => {
      process.env.TEST_SECRET_KEY = 'resolved-value';
      try {
        expect(store.resolveSecretValue({ $env: 'TEST_SECRET_KEY' })).toBeUndefined();
      } finally {
  ...
  describe('resolveAuthConfig', () => {
    it('does not resolve a bearer token env ref', () => {
  ...
  describe('listServiceNames', () => {
    it('should list stored service names', async () => {
      await store.setServiceSecret('svc1', {
        type: 'bearer',
        token: 'a',
        service_origin: 'https://one.example.test',
      });
      await store.setServiceSecret('svc2', {
        type: 'bearer',
        token: 'b',
        service_origin: 'https://two.example.test',
      });
