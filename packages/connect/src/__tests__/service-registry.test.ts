...
import { getServiceSecrets, setServiceSecret } from '../fetch/secrets-store.js';
...
describe('service-registry', () => {
  ...
  describe('addService', () => {
    ...
    it('purges a credential before a service name is rebound to another origin', async () => {
      await registry.addService('test', {
        base_url: 'https://old.example.test',
        auth_type: 'bearer',
      });
      await setServiceSecret('test', {
        type: 'bearer',
        token: 'old-token',
        service_origin: 'https://old.example.test',
      });
      ...
      expect(await getServiceSecrets('test')).toBeUndefined();
    });
  });
  ...
  describe('URL patterns', () => {
    ...
    it('should throw when adding pattern for nonexistent service', async () => {
      await expect(registry.addUrlPattern('api.example.com', 'nonexistent')).rejects.toThrow(
        'not found'
      );
    });
