...
type FetchImplementation = (url: string, options?: UndiciRequestInit) => Promise<UndiciResponse>;

let testFetchImplementation: FetchImplementation | undefined;

/** Inject a transport in tests without changing the production undici pairing. */
export function setFetchImplementationForTests(next?: FetchImplementation): void {
  if (!process.env.VITEST) {
    throw new Error('Fetch implementation overrides are test-only.');
  }
  testFetchImplementation = next;
}
...
function isResponseLike(value: unknown): value is ResponseLike {
  return (
    !!value &&
    typeof value === 'object' &&
    typeof (value as ResponseLike).headers?.get === 'function'
  );
}
...
export class RateLimiter {
    ...
    while (state.queue.length > 0 && state.active < this.config.per_domain) {
      ...
      if (next) {
        if (next.signal?.aborted) {
          next.cleanup?.();
          next.reject(
            Object.assign(new Error('Request was aborted while rate-limited.'), {
              name: 'AbortError',
            })
          );
          continue;
  ...
  async acquire(url: string, signal?: AbortSignal): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      const item: DomainState['queue'][number] = { resolve, reject, signal };
      if (signal) {
        const onAbort = (): void => {
  ...
  release(url: string, response?: ResponseLike): void {
    const domain = this.getDomain(url);
    const state = this.getState(domain);
  ...
  private parseRetryAfter(response: ResponseLike): number | null {
  ...
  async execute<T>(url: string, fn: () => Promise<T>, signal?: AbortSignal): Promise<T> {
...
export async function rateLimitedFetch(
  ...
  return globalRateLimiter.execute(
    url,
    () => (testFetchImplementation ?? undiciFetch)(url, options),
