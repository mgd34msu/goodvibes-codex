...
export async function resolveService(nameOrUrl: string): Promise<ResolvedService | undefined> {
  ...
  const auth =
    rawAuth &&
    serviceOrigin &&
    new URL(config.base_url).protocol === 'https:' &&
    rawAuth.service_origin === serviceOrigin &&
    config.auth_type !== undefined &&
    config.auth_type !== 'none' &&
    rawAuth.type === config.auth_type
      ? resolveAuthConfig(rawAuth)
      : undefined;
