 ...
 *    `write: true` AND the registered connection permits writes;
 *  - drivers load only from prepared plugin dependencies and remote drivers
 *    require verified TLS; SELECTs get an auto-LIMIT.
 */
...
import { getGlobalSecret } from '../fetch/secrets-store.js';
...
async function resolveTarget(
  ...
  _mode: TrustMode
): Promise<{ url: string; allowWrites: boolean } | { error: string }> {
  if (input.connection) {
    const conn = getConnection(input.connection);
    if (!conn) {
      return {
        error:
          `Connection "${input.connection}" is not registered. Register it with the ` +
          'interactive GoodVibes control utility.',
      };
    }
    const url = await getGlobalSecret(conn.secret_ref);
    if (!url) {
      return {
        error: `Connection "${input.connection}" has no readable credential handle. Re-register it.`,
      };
    }
    return { url, allowWrites: conn.allow_writes === true };
  }
  return { error: 'db_query requires a registered `connection` name.' };
}
...
export async function handleDbQuery(args: unknown): Promise<CallToolResult> {
  ...
  if (input.query.length > 256_000) {
    return fail('SQL statement exceeds the 256 KiB input limit.');
  }
  if (input.params && (!Array.isArray(input.params) || input.params.length > 10_000)) {
    return fail('`params` must be an array with at most 10,000 entries.');
  }

  const target = await resolveTarget(input, mode);
  if ('error' in target) {
    return fail(target.error);
  }

  const statement = classifyStatement(input.query);
  if (!statement.allowed) {
    return fail(statement.reason);
  }

  let connectionInfo: DatabaseConnectionInfo;
  try {
    connectionInfo = parseConnectionUrl(target.url);
  } catch (error) {
    return fail(
      error instanceof Error ? error.message : 'Connection URL policy validation failed.'
    );
  }
  ...
  const isWrite = statement.kind === 'write';
  ...
  if (isWrite && wantsWrite && !target.allowWrites) {
    return fail(
      'Writes are not permitted on this target. Register the connection with allow_writes:true, or ' +
        'change its grant through the interactive GoodVibes control utility.'
    );
  }
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > 1000) {
    return fail('`limit` must be an integer from 1 through 1000.');
  }
