...
export function isDestinationAllowed(
  ...
  if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') {
    return { allowed: false, reason: `Unsupported destination scheme '${parsed.protocol}'.` };
  }
  if (parsed.username || parsed.password) {
    return { allowed: false, reason: 'Destination URLs may not contain user-info credentials.' };
  }
  ...
  const allowlisted = policy.allowlist.some(entry => {
    if (entry.includes('://')) {
      return originOf(entry) === origin;
    }
    return defaultPort && entry.toLowerCase().replace(/\.$/, '') === host;
  });
  if (allowlisted) {
    return { allowed: true };
  }
  ...
  return {
    ...
    reason:
      ...
      `allowlist. Register the service or add the destination through the interactive control utility, or ` +
 ...
 * @param auth - the resolved auth used for the request
 */
