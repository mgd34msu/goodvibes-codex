...
export function createServer(onActivity?: () => void): Server {
  const server = new Server(
    { name: SERVER_NAME, version: SERVER_VERSION },
    {
      capabilities: { tools: {} },
      instructions:
        'Use only services and database connections registered by the user through the interactive GoodVibes control utility. ' +
        'The service tool is read-only. HTTP and SQL writes require explicit out-of-band grants and per-call write intent.',
    }
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async request => {
    ...
    return toCallToolResult(errorEnvelope(`Unknown tool: ${request.params.name}.`, { mode }));
  });
