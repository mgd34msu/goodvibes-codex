...
State updates are lock-protected and use same-directory atomic replacement. Budget/tag mutations require an exact session ID unless the host supplies one, and export parents are checked against symlink escape before writing.

Installed launchers infer the Codex home from their cache path because the host does not propagate path overrides; see [state and privacy](state-and-privacy.md#environment-override-limitation).
